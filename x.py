...
def handle_video(message):
    try:
        ...
        url = 'https://image-to-text9.p.rapidapi.com/ocr?url=' + f'file://{os.path.abspath("merged_frame.jpg")}'
        

        # Make OCR API request using file_content
        response = requests.get(url, headers=headers, files={'image': file_content})
        text = response.text
