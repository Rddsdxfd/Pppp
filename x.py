...
def handle_video(message):
    try:
        ...
        # Make sure that the merged_frame variable is a valid NumPy array
        if not isinstance(merged_frame, np.ndarray):
            raise ValueError("merged_frame is not a valid NumPy array")

        if merged_frame.ndim != 3:
            raise ValueError("merged_frame must have 3 dimensions")

        if merged_frame.shape[2] != 3:
            raise ValueError("merged_frame must have 3 channels")

        if merged_frame.dtype != np.uint8:
            raise ValueError("merged_frame must have a dtype of np.uint8")
        ...
        url = 'https://image-to-text9.p.rapidapi.com/ocr?url=' + message.video.file_id
